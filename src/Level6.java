...
public class Level6 extends Level implements KeyListener, ActionListener, MouseListener, MouseMotionListener  {
	...
	private void drawInfo(Graphics2D g2d) {
		...
		String s = "Note: Dashed boxes indicate the platform's bounds";
	...
	private void addPlatforms() {
		...
		platforms.add(new Platform(env.getWidth() - 100, env.getHeight() - 20, 100, height, Platform.NORMAL));
		platforms.add(new Platform(100, env.getHeight() - 20, 50, height, Platform.MOVING, Platform.HORIZONTAL,
				env.getWidth() - 100));
	}
	private void addStars() {
		//Format
		//new StarPolygon(int x, int y, int size, (2*size)/5, int vertexes, double startAngle)
		int size = 10;
		int startAngle = 55;
		int vertexes = 5;
		stars.add(new StarPolygon(env.getWidth() - (50 - size/2), env.getHeight() - 20 - size,
				size, (2*size)/5, vertexes, startAngle));
	}
