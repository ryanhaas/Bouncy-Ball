...
public class ComponentMover extends MouseAdapter {
	private Insets dragInsets = new Insets(-100, -100, 0, 0);
	...
	private Dimension getBoundingSize(Component source) {
		if (source instanceof Window) {
			...
			int width = 0;
			int height = 0;
			for (int x = 0; x < env.getScreenDevices().length; x++) {
				width += env.getScreenDevices()[x].getDisplayMode().getWidth();
				height += env.getScreenDevices()[x].getDisplayMode().getHeight();
			}
			Rectangle bounds = new Rectangle(0, 0, 3 * width / 2, height);
